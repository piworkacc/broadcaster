...
const onConnection = (socket, io) => {
  ...
  socket.on('message:send', async ({ message, room, user }) => {
    if (user.ok) {
      ...
      io.emit('message:error', { newMessage: defaultErrorMessaage, error: true });
      console.log(`User: ${headers.host} ${user} - ошибка сообщение " ${defaultErrorMessaage.message} " ошибка:`);
    }
  });
